...
#define	keepWDTalive system_soft_wdt_stop
#endif
...
//	if (p<0x20000000 || p>=0x60000000) return -1;
	if(p < 0x20000000 || p > 0x60009e00 || (p >= 0x60002000 && p < 0x60008000)) return -1;
	return *i>>((p&3)*8);
}
...
//	if (p<0x20000000 || p>=0x60000000) return;
	if(p < 0x20000000 || p > 0x60009e00 || (p >= 0x60002000 && p < 0x60008000)) return;
...
static int ATTR_GDBFN validWrAddr(int p) {
	...
	if (p>=0x60009800 && p<0x60009e00) return 1;
	return 0;
}
...
void ATTR_GDBFN gdbstub_handle_debug_exception() {
	...
	system_soft_wdt_restart(); //	ets_wdt_enable();
}
...
static void ATTR_GDBFN uart_hdlr(void *arg, void *frame) {
	...
	if (doDebug) {
		...
		//Copy any changed registers back to the frame the Xtensa HAL uses.
		os_memcpy(frame, &gdbstub_savedRegs, 19*4);
		WRITE_PERI_REG(UART_INT_ENA(0), UART_RXFIFO_FULL_INT_ENA);
	}
}
